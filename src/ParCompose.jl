...
(A::ParCompose{D,R,L,Parametric,F})(θ::AbstractVector{<:Number}) where {D,R,L,F} =
    ParCompose([i ∈ A.slots ? op(θ[range]) : op for (i, (op, range)) in enumerate(zip(A.ops, A.ranges))]...)
